...
- [ ] 1.1 Extend `src/composables/useTheme.ts` with `useThemePreference()`
...
- [ ] 1.2 `tests/composables/use-theme-preference.spec.ts`: shared
...
- [ ] 2.1 `src/components/VdTree.vue`: move `cascade` (and only
      `cascade`) into `withDefaults(defineProps<…>(), { cascade: true })`;
      delete the dead `props.cascade ?? true` expression.
- [ ] 2.2 Extend `tests/components/vd-tree.spec.ts`: default-mount parent
      check cascades to children; `:cascade="false"` opt-out does not.
- [ ] 2.3 Write the behavior-change note (default flip + escape hatch)
      into the changelog entry (task 8.1) and migration notes.

## 3. CSS-contract components

- [ ] 3.1 `VdBreadcrumb.vue`: items/slot rendering, current-item
      `aria-current` + `.vd-breadcrumb-current`, separator + size
      modifier classes; no listeners.
- [ ] 3.2 `tests/components/vd-breadcrumb.spec.ts`.
- [ ] 3.3 `VdFooter.vue`: columns/dark/size modifiers, default +
      copyright slots; no listeners.
- [ ] 3.4 `tests/components/vd-footer.spec.ts`.
- [ ] 3.5 `VdFab.vue`: class mapping (size/variant incl. `error`→`danger`
...
- [ ] 3.6 `tests/components/vd-fab.spec.ts` incl. speed-dial open/close
      paths.

## 4. VdNavbar

- [ ] 4.1 `VdNavbar.vue` ← `framework/js/components/navbar.js` (305):
      ...
      cleanup on unmount.
- [ ] 4.2 `tests/components/vd-navbar.spec.ts`: toggle round-trip with
...
- [ ] 5.1 `src/composables/useClickOutside.ts` ← vd2 donor (50 lines),
      carried with signature intact (capture-phase pointerdown, deferred
      attach, enable/disable + dispose detach).
- [ ] 5.2 `tests/composables/use-click-outside.spec.ts`: outside fires,
      inside/pre-attach ignored, disable + unmount detach.
- [ ] 5.3 `VdThemeSwitcher.vue` ← `theme-switcher.js` (466): menu mode
...
- [ ] 5.4 `tests/components/vd-theme-switcher.spec.ts`: selection applies
      + persists, cycle order, keyboard contract, cross-component sync
      with the singleton.
- [ ] 5.5 `VdThemeCustomizer.vue` — promote the vd2 donor (236 lines):
...
- [ ] 5.6 `tests/components/vd-theme-customizer.spec.ts`: swatch/radius/
      font writes (attribute + storage), palette opt-in, close paths,
      reset, window-event open, unmount listener cleanup.

## 6. Doc search

- [ ] 6.1 `src/composables/useDocSearch.ts` ← `doc-search.js` (1016),
...
- [ ] 7.1 `src/composables/useLazyLoad.ts` ← `lazy-load.js` (425):
      ...
      disposal.
- [ ] 7.2 `tests/composables/use-lazy-load.spec.ts` (mock IO + fetch):
      once-only callback, load lifecycle events + injection, unsafe URL
      + failure paths, sanitization, disposal.
- [ ] 7.3 `src/composables/useGrid.ts` ← `grid.js` (290): per-container
...
- [ ] 7.4 `css/core/grid.css`: add the `[data-grid="fibonacci"]` rule
...
- [ ] 7.5 `tests/composables/use-grid.spec.ts`: toggle round-trip +
...
- [ ] 8.1 `src/index.ts`: export the 7 components, 4 composables,
...
- [ ] 8.4 Flag the new public surface + the `data-grid` CSS hook for
      vd3-docs (docs sync happens in the docs repo's own change).
- [ ] 8.5 `openspec validate vd3-new-components --strict` green.
